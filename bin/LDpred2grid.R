...
fwrite(sumstats, 'ldpred2grid_output.txt')
...
# Debug message
cat("Prepared data for grid model \n")
...
# Debug message
cat("Get adjusted beta from grid model \n")

## Obtain Model PRS (Validation set)
# calculate PRS for  samples
pred_grid <- big_prodMat( G, beta_grid, ind.col = df_beta$`_NUM_ID_`)

## Obtain scores For validation data                          
...
# Debug message
cat("Obtain scores For validation data \n")

## Visualization of grid glm-z (For validation set)
library(ggplot2)

pdf("LDpred2 grid glm-z.pdf")
ggplot(grid.param, aes(x = p, y = score, color = as.factor(h2))) +
theme_bigstatsr() +
geom_point() +
geom_line() +
scale_x_log10(breaks = 10^(-5:0), minor_breaks = grid.param$p) +
facet_wrap(grid.param$sparse, labeller = label_both) +
labs(y = "GLM Z-Score", color = "h2") +
theme(legend.position = "top", panel.spacing = unit(1, "lines"))
dev.off()

# Debug message
cat("Choose the best model \n")
...
write.table(reg.dat, "LDpred2_grid.txt", sep = "\t", row.names = FALSE, col.names = TRUE)
(result <- data.table(grid = max.r2 - null.r2$R2, null = null.r2$R2))

# Debug message
cat("Grid results was written as LDpred2_grid.txt file \n")
